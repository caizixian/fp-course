...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n a = sequence (take n (repeat a))
...
-- List (f Bool) by map
-- List ((f Bool), a)
-- filtering pred lst = sequence (map (\a b -> ) (zip (map pred lst) lst))
--  where 
filtering _ Nil = pure Nil
-- filter pred t :: f (List a)
-- pure h :: f a
-- pred h :: f Bool
-- fun Bool -> a -> List a
-- lift2 fun
filtering pred (h:.t) = lift3 add2lst (pred h) (pure h) (filtering pred t)
  where add2lst ok item lst = if ok then item:.lst else lst
