...
satisfy pred = do
  r <- character
  if pred r then pure r else unexpectedCharParser r
...
smokerParser = do
  a <- is 'y' ||| is 'n'
  pure (if a == 'y' then True else False)
...
personParser = do
  ...
  _ <- spaces1
  phone <- phoneParser
  pure (Person age first_name surname smoker phone)
